...
| [consoleCanvasCapture.test.ts](../test/consoleCanvasCapture.test.ts) | Screenshotting the console tab once a second, which makes the browser hold a Windows display power request so the laptop can never auto-lock; plus the two ways reading the canvas instead can go wrong -- a blank WebGL read, and an unsized 300x150 canvas on the first tick |
