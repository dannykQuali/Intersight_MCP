...
Practicalities: recognition costs ~0.6-1.4s per frame, so the scan is bounded (`maxFrames`, default 25) and runs newest-first, stopping at `maxMatches`. Results are cached per frame, so repeat polling only OCRs genuinely new frames (measured: a repeat search went 2.1s to 0ms). Prominent text — dialogs, headings, full-screen messages — reads reliably; small UI chrome often does not, so match on conspicuous words. OCR data is fetched on first use, and if that is unavailable the tool reports `ocrUnavailable` rather than failing.
