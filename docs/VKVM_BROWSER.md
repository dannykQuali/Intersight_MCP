...
- **A starting recorder ADOPTS the frames it finds.** Their capture times come from file mtimes and they are labelled `reason: 'adopted'` — change ratios are never invented, because a fabricated `0` would be indistinguishable from a measured one. The sequence continues past the highest existing frame, so nothing is overwritten.
- **Inherited frames are exempt from the retention clock.** The first version of adoption pruned on attach, and a field report caught it: 56 frames inherited from the previous run, then evicted immediately against the new session's 4-hour window — *"attaching can silently destroy the history you attached to inspect."* Retention exists to bound a capture that keeps **growing**; an inherited set is finite and is the only record of what the machine did before anyone was watching. So the age clock only touches frames this recorder captured. The `maxFrames` cap is the safety valve (exemption cannot mean unbounded disk — every restart converts that run's live frames into inherited ones), and when it does take inherited frames, `adoptedFramesEvicted` and the note say so explicitly rather than burying it in the general eviction count.
